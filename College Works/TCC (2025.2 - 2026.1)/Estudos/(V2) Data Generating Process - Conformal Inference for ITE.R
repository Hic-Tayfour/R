...
# ============================================================================
# NOVO: Estimar Erros Padrão dos Resíduos
# ============================================================================
...
# Resíduos ABSOLUTOS
# Resíduos ABSOLUTOS (com dplyr explícito)
trn_0_res <- trn_0_with_D %>%
  dplyr::mutate(delta = abs(D - tau0_hat_trn)) %>%
  dplyr::select(-Y, -Y0, -Y1, -tau_true, -propensity_score, -treatment, -D)

trn_1_res <- trn_1_with_D %>%
  dplyr::mutate(delta = abs(D - tau1_hat_trn)) %>%
  dplyr::select(-Y, -Y0, -Y1, -tau_true, -propensity_score, -treatment, -D)

# Treinar Random Forest para prever |resíduos|
rf_sigma0 <- ranger(delta ~ ., data = trn_0_res, num.trees = 500)
rf_sigma1 <- ranger(delta ~ ., data = trn_1_res, num.trees = 500)
...
# NOVO: Erros padrão estimados (combinação ponderada)
...
cat(sprintf("Resíduo médio (cal): %.3f [%.3f, %.3f]\n", 
            mean(sigma_hat_cal), min(sigma_hat_cal), max(sigma_hat_cal)))

cat(sprintf("Resíduo médio (test): %.3f [%.3f, %.3f]\n\n", 
            mean(sigma_hat_tst), min(sigma_hat_tst), max(sigma_hat_tst)))

# Conformal Prediction

# Resíduos de não-conformidade NORMALIZADOS (ITE)
...
cat("Largura média:", mean(upper - lower), "\n")
