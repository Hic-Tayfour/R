...
db <- star |>
  as_tibble() |>
  drop_na() |>
  mutate(t = if_else(cltype == "small", 1, 0)) |>
  rename(y = math) |>
  select(-id, -cltype)
...
tibble(id = 1:nrow(tst1), lower = lower_ite, upper = upper_ite) |>
  ...
  ggplot(aes(ite_est, id, color = status)) +
    ...
    labs(title = "Efeitos Individuais de Tratamento (ITE)",
         subtitle = "Intervalos conformais (90%) estimados via Random Forest Padrão em uma amostra de 30 alunos.",
         caption = "Fonte: Elaboração própria com dados do projeto STAR (pacote mlmRev).",
         x = "Estimativa do Efeito Contrafactual nas Notas de Matemática",
         y = NULL) +
    theme_econ_base() +
    theme(
...
tibble(id = 1:nrow(tst1), lower = lower_ite, upper = upper_ite) |>
  ...
  scale_color_manual(values = c("Positivo" = unname(pal["blue1"]), 
                                ...
                                "Negativo" = unname(pal["econ_red"]))) +
  labs(title = "Efeitos Individuais de Tratamento (ITE)",
       subtitle = "Intervalos conformais (90%) estimados via Quantile Regression Forest em uma amostra de 30 alunos.",
       caption = "Fonte: Elaboração própria com dados do projeto STAR (pacote mlmRev).",
       x = "Estimativa do Efeito Contrafactual nas Notas de Matemática",
       y = NULL) +
  theme_econ_base() +
...
tibble(id = 1:nrow(tst1), lower = lower_ite, upper = upper_ite) |>
  ...
  scale_color_manual(values = c("Positivo" = unname(pal["blue1"]), 
                                ...
                                "Negativo" = unname(pal["econ_red"]))) +
  labs(title = "Efeitos Individuais de Tratamento (ITE)",
       subtitle = "Intervalos conformais adaptativos (90%) estimados via BART em uma amostra de 30 alunos.",
       caption = "Fonte: Elaboração própria com dados do projeto STAR (pacote mlmRev).",
       x = "Estimativa do Efeito Contrafactual nas Notas de Matemática",
       y = NULL) +
  theme_econ_base() +
...
tibble(id = 1:nrow(tst1), lower = lower_ite, upper = upper_ite) |>
  ...
  scale_color_manual(values = c("Positivo" = unname(pal["blue1"]), 
                                ...
                                "Negativo" = unname(pal["econ_red"]))) +
  labs(title = "Efeitos Individuais de Tratamento (ITE)",
       subtitle = "Intervalos conformais (90%) estimados via Random Forest Normalizada em uma amostra de 30 alunos.",
       caption = "Fonte: Elaboração própria com dados do projeto STAR (pacote mlmRev).",
       x = "Estimativa do Efeito Contrafactual nas Notas de Matemática",
       y = NULL) +
  theme_econ_base() +
...
tibble(id = 1:nrow(tst1), lower = lower_ite, upper = upper_ite) |>
  ...
  scale_color_manual(values = c("Positivo" = unname(pal["blue1"]), 
                                ...
                                "Negativo" = unname(pal["econ_red"]))) +
  labs(title = "Efeitos Individuais de Tratamento (ITE)",
       subtitle = "Intervalos conformais (90%) estimados via CatBoost Quantílico em uma amostra de 30 alunos.",
       caption = "Fonte: Elaboração própria com dados do projeto STAR (pacote mlmRev).",
       x = "Estimativa do Efeito Contrafactual nas Notas de Matemática",
       y = NULL) +
  theme_econ_base() +
