...
w_cal <- with(cal0[order(R), ], e / (1 - e))
w_tst <- with(tst1, e / (1 - e))
...
# Modelo Quantile Regresion Forest ----

rf0 <- ranger(y ~ ., data = trn |> filter(t == 0) |> select(-e, -t, -y0, -y1), quantreg = TRUE)
...
q_hat_cal <- predict(rf0, data = cal0, type = "quantiles", quantiles = c(alpha_out, 1 - alpha_out))$predictions
q_hat_tst <- predict(rf0, data = tst1, type = "quantiles", quantiles = c(alpha_out, 1 - alpha_out))$predictions
...
trn0_mad <- trn |> filter(t == 0) |> select(-e, -t, -y0, -y1) |> mutate(res_abs = abs(y - rf0$predictions)) |> 
  select(-y)
...
(metricas_consolidadas <- tibble(
  ...
  cobertura_tau  = c(cov_tau_rf, cov_tau_qrf, cov_tau_bart, cov_tau_rfn, cov_tau_cbq),
