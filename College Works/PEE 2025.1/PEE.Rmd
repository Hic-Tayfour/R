...
date: |
  <div style="text-align: center; margin-top: 20px;">
    <p><strong>São Paulo – Fevereiro/Maio de 2025</strong></p>
  </div>
...
Este trabalho poderá ser encontrado no GitHub [PEE 2025.1 - Grupo 7](https://github.com/Hic-Tayfour/R/tree/main/College%20Works/PEE%202025.1). A estrutura do repositório está organizada para garantir reprodutibilidade total da análise.

## 📁 Estrutura de Arquivos

A pasta `Data` contém 5 arquivos `.xlsx` fundamentais:

1. **CBIDta.xlsx**: Dados sobre independência dos bancos centrais (CBIE) — Romelli (2022, 2024) — cobrindo 155 países entre 1923 e 2023.
2. **InflationForecast(FMI).xlsx**: Projeções de inflação do FMI.
3. **rate_macrobond.xlsx**: Taxas de juros nominais coletadas via Macrobond.
4. **target(eikon).xlsx**: Metas de inflação obtidas via Eikon.
5. **target(macrobond).xlsx**: Metas de inflação obtidas via Macrobond (prioritárias para países da Zona do Euro).

Além disso, os seguintes scripts e documentos estão incluídos:

- **PEE.R**: Código principal com etapas de importação, limpeza, cálculo e estimativas.
- **PEE.Rmd**: Documento R Markdown com a estrutura analítica e narrativa do trabalho.
- **Base de Dados.md**: Script para download automatizado dos dados via GitHub.
- **README.md**: Descrição completa do projeto, metodologia e orientações de execução.
